To reduce redundant network calls and improve the performance of your Angular application, you can leverage state management techniques or shared data services. Since your `supplier-list` component already fetches the full list of suppliers, passing or making that locally available data accessible to the `supplier-details` and `supplier-info-update` (edit) components will eliminate unnecessary API calls.
...
You can modify your `SupplierService` or create a new dedicated state service to cache the list of suppliers using RxJS `BehaviorSubject`.
*   **How it works:** When the list is fetched in `/suppliers`, store it in the service via a `BehaviorSubject`. 
...
*   **How it works:** In `supplier-list` (or `supplier-details`), use the router's `navigate` method with a `state` object containing the data:
    ```typescript
    this.router.navigate(['/suppliers', supplier.id], { state: { supplierData: supplier } });
    ```
...
*   **How it works:** When you fetch the list, an action is dispatched to store all suppliers centrally. The details and edit components map their UI state via selectors.
*   **Enterprise Fit:** This is the most robust enterprise-level solution for data flow. If a user tries to access `/suppliers/id/edit` and the store is empty (e.g., direct link visit), you can configure NgRx Effects or guards to transparently fetch just the missing entity. The drawback is the boilerplate code.
...
1. **Combine Stateful Service with Route Resolvers:** Keep a cache of the suppliers retrieved during the list view.
